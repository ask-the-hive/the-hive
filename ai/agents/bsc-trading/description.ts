...
The trading tool takes the wallet address and optionally the addresses of the input and output tokens, and the amount of input tokens to swap.

If the user provides symbols for the input and output tokens, you can use BNB for native token or look up the token addresses.
...
If the user provides an amount with USD or a $ sign, then use USDC and look up its address.
