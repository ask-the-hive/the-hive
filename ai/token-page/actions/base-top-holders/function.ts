...
export async function getBaseTokenPageTopHolders(
  token: TokenChatData,
  args: TokenPageTopHoldersArgumentsType,
): Promise<TokenPageActionResult<TokenPageTopHoldersResultBodyType>> {
  try {
    ...
    const top5HoldersPercent = sortedHolders
      .slice(0, 5)
      .reduce((acc, curr) => acc + curr.percentage, 0);
    const top10HoldersPercent = sortedHolders
      .slice(0, 10)
      .reduce((acc, curr) => acc + curr.percentage, 0);
    const top20HoldersPercent = sortedHolders
      .slice(0, 20)
      .reduce((acc, curr) => acc + curr.percentage, 0);
    ...
    // Averages
    const avgTop10Holding =
      sortedHolders.slice(0, 10).length > 0
        ? top10HoldersPercent / sortedHolders.slice(0, 10).length
        : 0;
    ...
    return {
      message: toUserFacingErrorTextWithContext("Couldn't analyze holder data right now.", error),
    };
  }
}
