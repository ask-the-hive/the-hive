import { getTokenPriceHistory, getTokenOverview, PriceHistoryItem } from "@/services/birdeye";

import type { TokenPagePriceAnalysisResultBodyType, TokenPagePriceAnalysisArgumentsType } from "../price-analysis/types";
import type { SolanaActionResult } from "../../../solana/actions/solana-action";
import type { TokenChatData } from "@/types";

export async function getBSCTokenPagePriceAnalysis(token: TokenChatData, _: TokenPagePriceAnalysisArgumentsType): Promise<SolanaActionResult<TokenPagePriceAnalysisResultBodyType>> {
    try {
        ...
        const dailyVolatility = dailyChanges.reduce((sum, change) => sum + change, 0) / dailyChanges.length;
        ...
        const avgRecentChange = recentDailyChanges.reduce((sum, change) => sum + change, 0) / recentDailyChanges.length;
        const weeklyVolatility = Math.sqrt(recentDailyChanges.reduce((sum, change) => sum + Math.pow(change - avgRecentChange, 2), 0) / recentDailyChanges.length);
...
6. Technical Levels:
   ...
   - Resistance: ${resistance[0].toLocaleString('en-US', { style: 'currency', currency: 'USD' })} and ${resistance[1].toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`,
        ...
        console.error(error);
        return {
            message: `Error analyzing price data for BSC token: ${error}`,
        };
    }
} 