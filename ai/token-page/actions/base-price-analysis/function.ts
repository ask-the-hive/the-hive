...
export async function getBaseTokenPagePriceAnalysis(
  token: TokenChatData,
  _: TokenPagePriceAnalysisArgumentsType,
): Promise<TokenPageActionResult<TokenPagePriceAnalysisResultBodyType>> {
  try {
...
The token is showing ${volatilityLevel.toLowerCase()} volatility with a ${priceTrend.toLowerCase()} price trend over the last 24 hours.`,
      body: {
        ...
        trendAnalysis: {
          direction:
            percentChange24h > 0 ? 'bullish' : percentChange24h < 0 ? 'bearish' : 'sideways',
