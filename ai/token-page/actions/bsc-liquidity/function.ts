...
import type { TokenPageLiquidityResultBodyType, TokenPageLiquidityArgumentsType } from "../liquidity/types";
import type { SolanaActionResult } from "../../../solana/actions/solana-action";
import type { TokenChatData } from "@/types";
import type { MarketsResponseData, MarketItem } from "@/services/birdeye/types";

export async function getBSCTokenPageLiquidity(token: TokenChatData, _: TokenPageLiquidityArgumentsType): Promise<SolanaActionResult<TokenPageLiquidityResultBodyType>> {
    try {
        ...
        const top3PoolsShare = sortedMarkets.slice(0, 3).reduce((acc, market) => acc + market.liquidity, 0) / totalLiquidityUSD * 100;
...
${healthScore < 40 ? "⚠️ Warning: This token has limited liquidity which may result in high slippage and difficulty exiting positions." : ""}`,
        ...
        console.error(error);
        return {
            message: `Error analyzing liquidity for BSC token: ${error}`,
        };
    }
}
