...
export async function getBaseTokenPageTradingActivity(
  token: TokenChatData,
  _: TokenPageTradingActivityArgumentsType,
): Promise<TokenPageActionResult<TokenPageTradingActivityResultBodyType>> {
  try {
    ...
    const topTraders =
      topTradersResult.body?.topTraders?.map((trader) => ({
