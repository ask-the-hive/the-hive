...
// Using the same types as the BSC trading activity function
...
export async function getBaseTokenPageTradingActivity(token: TokenChatData, _: TokenPageTradingActivityArgumentsType): Promise<TokenPageActionResult<TokenPageTradingActivityResultBodyType>> {
  try {
    // Use the existing Base top traders action
    const baseAction = new BaseTopTokenTradersAction();
    ...
    if (!overview) {
      return {
        message: `Could not find trading data for this Base token.`,
        body: {
          topTraders: [],
          volume24h: 0,
          volumeChange: 0,
    ...
    const topTradersResult = await baseAction.func({
      search: token.address,
      timeFrame: TopTradersByTokenTimeFrame.TwentyFourHours
    });
    
    // Ensure we have top traders and format their data
    const topTraders = topTradersResult.body?.topTraders?.map(trader => ({
      address: trader.owner || 'Unknown',
      volume: trader.volume || 0
    })) || [];
    
    // Calculate metrics using the correct property names
