...
): Promise<BscActionResult<TopHoldersResultBodyType>> {
  try {
    ...
    try {
      ...
      const topHolders = await getTokenTopHolders(token.address);
      
      if (!topHolders || topHolders.length === 0) {
        return {
          message: `No holders found for ${token.name} (${token.symbol}) on BSC`,
        };
      }
      ...
      return {
        message: `Error getting top holders for ${token.name} (${token.symbol}) on BSC: ${error}`,
      };
    }
  } catch (error) {
    console.error("Error searching for token:", error);
    return {
      message: `Error getting top holders on BSC: ${error}`,
    };
  }
} 