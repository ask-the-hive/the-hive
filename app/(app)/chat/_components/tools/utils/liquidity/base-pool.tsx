...
import { Card, Button, Separator, Input, Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui'
...
const BasePool: React.FC<Props> = ({ pair }) => {
    ...
    async function connectWallet() {
        try {
            ...
            setIsWalletConnected(true);
            setStatus("Wallet connected to Base!");
        } catch (err: any) {
    ...
    useEffect(() => {
        async function loadPoolReserves() {
            try {
                ...
                const response = await fetch(`/api/moralis/reserves?pairAddress=${pair.pair_address}&chain=base`);
                ...
                const baseReserveNormalized = new Decimal(reserve0).div(new Decimal(10).pow(baseToken.token_decimals));
                const quoteReserveNormalized = new Decimal(reserve1).div(new Decimal(10).pow(quoteToken.token_decimals));
    ...
    useEffect(() => {
        const refreshReserves = async () => {
            try {
                ...
                if (err.message && !err.message.includes('missing revert data') && !err.message.includes('CALL_EXCEPTION')) {
    ...
    }, [pair.pair_address, baseToken.token_address, baseToken.token_decimals, quoteToken.token_decimals, baseAmount, quoteAmount]);
    ...
    async function approveTokens() {
        try {
            ...
            if (err.message && (err.message.includes('user rejected action') || err.message.includes('User denied transaction signature') || err.message.includes('ACTION_REJECTED'))) {
    ...
    async function addLiquidity() {
        try {
            ...
            const network = await provider.getNetwork();
            if (network.chainId !== BigInt(8453)) {
                setStatus("Please switch to Base network in your wallet first (MetaMask → Networks → Base)");
            ...
            if (quoteToken.token_address.toLowerCase() === "0x4200000000000000000000000000000000000006") {
                tx = await router.addLiquidityETH(
                    ...
                    parseUnits(Number(baseAmount).toFixed(Number(baseToken.token_decimals)), Number(baseToken.token_decimals)),
                ...
                tx = await router.addLiquidity(
                    ...
                    parseUnits(Number(baseAmount).toFixed(Number(baseToken.token_decimals)), Number(baseToken.token_decimals)),
                    parseUnits(Number(quoteAmount).toFixed(Number(quoteToken.token_decimals)), Number(quoteToken.token_decimals)),
                    0, // slippage handled by frontend
                    0, // slippage handled by frontend
                    await signer.getAddress(),
                    deadline
            ...
            if (err.message && (err.message.includes('TRANSFER_FROM_FAILED') || err.message.includes('missing revert data') || err.message.includes('CALL_EXCEPTION'))) {
                    ...
                    <div className="flex flex-col gap-4 py-4">
                        {!isWalletConnected ? (
                            ...
                            <>
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-medium">
                                        {quoteToken.token_symbol} Amount
                                    </label>
                                    <Input
                                        type="text"
