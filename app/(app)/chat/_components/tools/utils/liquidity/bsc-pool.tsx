...
import { Card, Button, Separator, Dialog, DialogContent, DialogHeader, DialogTitle, Input } from "@/components/ui";
...
const BscPool: React.FC<Props> = ({ pair }) => {
    ...
    // Check wallet connection on mount and when window.ethereum changes
    useEffect(() => {
    ...
    useEffect(() => {
        async function loadPoolReserves() {
            try {
                ...
                const response = await fetch(`/api/moralis/reserves?pairAddress=${pair.pair_address}&chain=0x38`);
