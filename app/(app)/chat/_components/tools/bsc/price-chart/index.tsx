...
import type { PriceChartResultType } from '@/ai/bsc/actions/token/price-chart/types';
...
const PriceChart: React.FC<Props> = ({ tool, prevToolAgent }) => {
    return (
        <ToolCard 
            ...
            result={{
                heading: (result: PriceChartResultType) => result.body 
                    ? `BSC Token Price Chart` 
                    : `Failed to fetch BSC token price chart`,
                body: (result: PriceChartResultType) => result.body 
                    ? <PriceChartBody 
