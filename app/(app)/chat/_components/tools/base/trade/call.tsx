    ...
    const fetchTokens = async () => {
      ...
      try {
        ...
        if (args.inputTokenAddress && args.inputTokenAddress !== 'ETH') {
          ...
          const isAddress =
            args.inputTokenAddress.startsWith('0x') && args.inputTokenAddress.length === 42;

          if (isAddress) {
            // It's an address, fetch metadata directly
            try {
              const response = await fetch(
                `/api/token/${args.inputTokenAddress}/metadata?chain=base`,
              );
              if (response.ok) {
            ...
            const response = await fetch(
              `/api/token/search?query=${encodeURIComponent(searchQuery)}&chain=base&search_mode=fuzzy`,
            );
        ...
        if (args.outputTokenAddress && args.outputTokenAddress !== 'ETH') {
          // Check if it's an address or symbol
          const isAddress =
            args.outputTokenAddress.startsWith('0x') && args.outputTokenAddress.length === 42;

          if (isAddress) {
            // It's an address, fetch metadata directly
            try {
              const response = await fetch(
                `/api/token/${args.outputTokenAddress}/metadata?chain=base`,
              );
              if (response.ok) {
                const metadata = await response.json();
                setOutputToken({
    ...
    try {
      ...
      if (quote.issues) {
        if (quote.issues.balance) {
          ...
          const tokenSymbol =
            token === '0x4200000000000000000000000000000000000006' ? 'ETH' : inputToken.symbol;
          ...
          throw new Error(
            `Insufficient balance: have ${actualFormatted} ${tokenSymbol}, need ${expectedFormatted} ${tokenSymbol}`,
          );
        }
