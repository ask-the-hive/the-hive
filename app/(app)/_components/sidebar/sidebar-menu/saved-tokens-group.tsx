...
const SavedTokensGroup: React.FC = () => {
  ...
  const [tokenPrices, setTokenPrices] = useState<
    Record<string, { price: number; priceChange24hPercent: number }>
  >({});
        ...
        savedTokens.map(async (token) => {
          try {
            ...
            const response = await fetch(
              `/api/token/${token.id}/overview?chain=${token.chain || 'solana'}`,
            );
                ...
                <SidebarMenuSubItem key={savedToken.id}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {tokenPrices[savedToken.id] &&
                          tokenPrices[savedToken.id].priceChange24hPercent !== null &&
                          tokenPrices[savedToken.id].priceChange24hPercent !== undefined && (
                            <span
                              className={cn(
                                'text-xs font-medium',
