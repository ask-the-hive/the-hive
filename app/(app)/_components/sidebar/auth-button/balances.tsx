...
const Balances: React.FC<Props> = ({ address, chain }) => {
  ...
  const { data: portfolio, isLoading: isPortfolioLoading } = usePortfolio(chainAddress, chain);
  ...
  if (chain === 'solana') {
    ...
    if (tokenAccountsError || nativeBalanceError) return <p>Error fetching balances</p>;
    ...
    return (
      <div className="flex flex-col gap-2 max-h-48 overflow-y-auto px-2">
        ...
        {tokenAccounts && tokenAccounts.length > 0 ? (
          ...
          <div className="text-sm text-muted-foreground px-1">No tokens found</div>
        )}
      </div>
    );
    ...
    if (isPortfolioLoading) return <Skeleton className="h-10 w-full" />;

    // BSC/Base balances
    if (!portfolio || !portfolio.items || portfolio.items.length === 0)
      return <div className="px-2 py-2 text-sm text-muted-foreground">No tokens found</div>;
    ...
    return (
      <div className="flex flex-col gap-2 max-h-48 overflow-y-auto px-2">
        {sortedTokens.map((token, index) => {
          ...
          if (
            chain === 'bsc' &&
            (token.name.toLowerCase().includes('wrapped bnb') ||
            ...
            chain === 'base' &&
            (token.name.toLowerCase().includes('wrapped eth') ||
          ...
          return (
            <div key={`token-${index}`} className="flex flex-row items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-muted-foreground">{token.uiAmount.toFixed(4)}</span>
              </div>
            </div>
          );
        })}
  ...
  return <div className="px-2 py-2 text-sm">No balances available</div>;
};
