...
const SearchBar: React.FC = () => {
  ...
  const sortTokens = (tokens: TokenSearchResult[]) => {
    ...
    const priorityTokens = tokens.filter((token) => currentPriorityAddresses.has(token.address));

    const nonPriorityTokens = tokens.filter(
      (token) => !currentPriorityAddresses.has(token.address),
    );
                        ...
                        >
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <div className="flex flex-row items-center gap-3 text-xs text-muted-foreground">
                              <span>
                                $
                                {(token.price || 0).toLocaleString(undefined, {
                                ...
                                {typeof token.price_change_24h_percent === 'number' && (
                                  ...
                                  >
                                    ({token.price_change_24h_percent > 0 ? '+' : ''}
                                    {token.price_change_24h_percent.toLocaleString(undefined, {
                              ...
                              <span className="text-muted-foreground">
                                Mcap:{' '}
                                {token.market_cap
                                  ? `$${token.market_cap.toLocaleString(undefined, { maximumFractionDigits: 0 })}`
                                  : '—'}
                              </span>
                            </div>
                          </div>
